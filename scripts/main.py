            ...
            with gr.Column(variant="panel"):
                ...
                with gr.Tab("Train Model"):
                    with gr.Column():
                        ...
                        df_eval_interval = gr.Number(label='eval_interval', default=10, precision=0, placeholder="evaluate on the valid set every interval epochs")
                        ...
                        df_seed = gr.Number(label='seed', default=0, precision=0, placeholder="seed")
                        df_iters = gr.Number(label='iters', default=10000, precision=0, placeholder="training iters")
                        ...
                        df_max_steps = gr.Number(label='max_steps', default=512, precision=0, placeholder="max num steps sampled per ray (only valid when using --cuda_ray)")
                        df_num_steps = gr.Number(label='num_steps', default=64, precision=0, placeholder="num steps sampled per ray (only valid when not using --cuda_ray)")
                        df_upsample_steps = gr.Number(label='upsample_steps', default=32, precision=0, placeholder="num steps up-sampled per ray (only valid when not using --cuda_ray)")
                        df_update_extra_interval = gr.Number(label='update_extra_interval', precision=0, default=16, placeholder="iter interval to update extra status (only valid when using --cuda_ray)")
                        df_max_ray_batch = gr.Number(label='max_ray_batch', default=4096, precision=0, placeholder="batch size of rays at inference to avoid OOM (only valid when not using --cuda_ray)")
                        df_albedo = gr.Checkbox(label='albedo', default=False, placeholder="only use albedo shading to train, overrides --albedo_iters")
                        df_albedo_iters = gr.Number(label='albedo_iters', default=1000, precision=0, placeholder="training iters that only use albedo shading")
                        ...
                        df_w = gr.Number(label='w', default=64, precision=0, placeholder="render width for NeRF in training")
                        df_h = gr.Number(label='h', default=64, precision=0, placeholder="render height for NeRF in training")
                        ...
                        df_gui_w = gr.Number(label='gui_w', default=800, precision=0, placeholder="GUI width")
                        df_gui_h = gr.Number(label='gui_h', default=800, precision=0, placeholder="GUI height")
